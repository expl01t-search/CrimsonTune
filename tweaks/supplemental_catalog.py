...
SUPPLEMENTAL_TWEAKS: list[SupplementalTweak] = [
    ...
    SupplementalTweak(
        id="nvidia_max_frame_latency",
        name="NVIDIA: MaxFrameLatency 1",
        description="MaxFrameLatency=1 на GPU-адаптерах — меньше input lag",
        category="nvidia",
        risk="medium",
        gpu_vendor=["nvidia"],
        source="SpeedGuide · NVIDIA",
        hint="Аналог Low Latency Ultra через реестр · перезагрузка рекомендуется",
        en_name="NVIDIA: MaxFrameLatency 1",
        en_description="MaxFrameLatency=1 on GPU adapters for lower input lag",
        en_hint="Registry alternative to Low Latency Ultra · reboot recommended",
    ),
    SupplementalTweak(
        id="nvidia_disable_runtime_pm",
        name="NVIDIA: runtime PM OFF",
        description="EnableRuntimePowerManagement=0 — GPU не уходит в энергосбережение",
        category="nvidia",
        risk="medium",
        requires_reboot=True,
        gpu_vendor=["nvidia"],
        source="Optimize #Expl01t · community",
        en_name="NVIDIA: Disable Runtime PM",
        en_description="EnableRuntimePowerManagement=0 on GPU adapters",
    ),
    SupplementalTweak(
        id="nvidia_driver_perf",
        name="NVIDIA: driver perf",
        description="Отключение slowdown + RmGpsEnableGlobalPerfOpt + DisplayPowerSaving OFF",
        category="nvidia",
        risk="medium",
        requires_reboot=True,
        gpu_vendor=["nvidia"],
        source="BoosterX · community",
        hint="nvlddmkm + NVTweak · больше энергопотребление",
        en_name="NVIDIA: Driver Performance",
        en_description="Disable clock slowdown and enable global perf optimization",
        en_hint="nvlddmkm + NVTweak · higher power draw",
    ),
    SupplementalTweak(
        id="msi_mode_high_priority",
        name="MSI Mode: High (GPU/USB/LAN)",
        description="MSISupported + DevicePriority High — как MSI Mode Utility v3",
        category="gaming",
        risk="medium",
        requires_reboot=True,
        gpu_vendor=["all"],
        source="MSI Mode Utility",
        hint="GPU, USB xHCI и Ethernet/Wi-Fi · перезагрузка обязательна",
        en_name="MSI Mode: High (GPU/USB/LAN)",
        en_description="MSISupported + DevicePriority High like MSI Mode Utility v3",
        en_hint="GPU, USB xHCI and Ethernet/Wi-Fi · reboot required",
    ),
    SupplementalTweak(
        id="ssd_enable_trim",
        name="SSD: включить TRIM",
        description="fsutil DisableDeleteNotify=0 на фиксированных томах",
        category="ssd",
        risk="safe",
        source="SSD Mini Tweaker",
        en_name="SSD: Enable TRIM",
        en_description="fsutil DisableDeleteNotify=0 on fixed volumes",
    ),
    SupplementalTweak(
        id="ssd_disable_superfetch_prefetch",
        name="SSD: Prefetch/Superfetch OFF",
        description="EnablePrefetcher=0 и EnableSuperfetch=0 — не нужны на SSD",
        category="ssd",
        risk="medium",
        requires_reboot=True,
        source="SSD Mini Tweaker",
        hint="Дублирует часть disable_sysmain / disable_prefetch",
        en_name="SSD: Disable Prefetch/Superfetch",
        en_description="EnablePrefetcher=0 and EnableSuperfetch=0 for SSD",
        en_hint="Overlaps disable_sysmain / disable_prefetch",
    ),
    SupplementalTweak(
        id="ssd_disable_boot_defrag",
        name="SSD: дефрагментация при загрузке OFF",
        description="BootOptimizeFunction Enable=N",
        category="ssd",
        risk="safe",
        source="SSD Mini Tweaker",
        en_name="SSD: Disable Boot Defrag",
        en_description="BootOptimizeFunction Enable=N",
    ),
    SupplementalTweak(
        id="ssd_disable_layout_ini",
        name="SSD: Layout.ini OFF",
        description="OptimalLayout + Prefetch scenario отключены",
        category="ssd",
        risk="safe",
        requires_reboot=True,
        source="SSD Mini Tweaker",
        en_name="SSD: Disable Layout.ini",
        en_description="Disable OptimalLayout and Prefetch scenario",
    ),
    SupplementalTweak(
        id="ssd_disable_defrag_service",
        name="SSD: служба дефрагментации OFF",
        description="defragsvc отключена",
        category="ssd",
        risk="medium",
        source="SSD Mini Tweaker",
        en_name="SSD: Disable Defrag Service",
        en_description="Disable defragsvc service",
    ),
    SupplementalTweak(
        id="ssd_disable_scheduled_defrag",
        name="SSD: плановая дефрагментация OFF",
        description="Задачи \\Microsoft\\Windows\\Defrag\\ отключены",
        category="ssd",
        risk="safe",
        source="SSD Mini Tweaker",
        en_name="SSD: Disable Scheduled Defrag",
        en_description="Disable \\Microsoft\\Windows\\Defrag\\ scheduled tasks",
    ),
    SupplementalTweak(
        id="ssd_disable_system_restore",
        name="SSD: защита системы OFF",
        description="DisableSR + Disable-ComputerRestore — освобождает место на SSD",
        category="ssd",
        risk="high",
        source="SSD Mini Tweaker",
        hint="Отключает точки восстановления · используйте бэкап CrimsonTune",
        en_name="SSD: Disable System Restore",
        en_description="DisableSR and Disable-ComputerRestore to save SSD space",
        en_hint="Disables restore points · use CrimsonTune backup",
    ),
    SupplementalTweak(
        id="ssd_disable_volume_indexing",
        name="SSD: индексирование дисков OFF",
        description="IndexingEnabled=false на фиксированных томах (Win32_Volume)",
        category="ssd",
        risk="medium",
        source="SSD Mini Tweaker",
        hint="Поиск Windows станет медленнее · дублирует disable_search_indexing частично",
        en_name="SSD: Disable Volume Indexing",
        en_description="IndexingEnabled=false on fixed volumes via WMI/CIM",
        en_hint="Windows Search slower · overlaps disable_search_indexing",
    ),
    SupplementalTweak(
        id="disable_nagle_nic_interfaces",
        name="Nagle OFF на NIC",
        description="TcpAckFrequency, TCPNoDelay, TcpDelAckTicks на активных адаптерах",
        category="network",
        risk="medium",
        requires_reboot=True,
        source="SpeedGuide · CS2 Ping Optimizer · Lutyk",
        hint="Точнее чем глобальный Nagle · при проблемах с файлами — откат",
        en_name="Disable Nagle on NIC",
        en_description="TcpAckFrequency, TCPNoDelay, TcpDelAckTicks on active adapters",
        en_hint="More precise than global Nagle · revert if file transfer issues",
    ),
    SupplementalTweak(
        id="psched_non_best_effort_limit",
        name="Psched: NonBestEffortLimit=0",
        description="Снимает резерв QoS bandwidth — CS2 Ping Optimizer / Win-Debloat",
        category="network",
        risk="safe",
        source="CS2 Ping Optimizer",
        entries=[
            (r"HKLM\SOFTWARE\Policies\Microsoft\Windows\Psched", "NonBestEffortLimit", 0, 20),
        ],
        en_name="Psched: NonBestEffortLimit=0",
        en_description="Removes QoS bandwidth reservation cap",
    ),
    SupplementalTweak(
        id="mmcss_games_full_priority",
        name="MMCSS Games: полный приоритет",
        description="Scheduling Category High, SFIO High, Priority 6, GPU 8 — SpeedGuide",
        category="gaming",
        risk="medium",
        requires_reboot=True,
        source="SpeedGuide · Win-Debloat",
        entries=[
            (
        ...
        en_name="MMCSS Games: Full Priority",
        en_description="Scheduling Category High, SFIO High, Priority 6, GPU 8",
    ),
    SupplementalTweak(
        id="tcp_ecn_gaming",
        name="TCP ECN (игры)",
        description="Explicit Congestion Notification — SpeedGuide gaming tweak",
        category="network",
        risk="medium",
        requires_reboot=False,
        source="SpeedGuide",
        hint="Может не работать со старыми роутерами · откат при проблемах с сетью",
        en_name="TCP ECN (gaming)",
        en_description="Enable Explicit Congestion Notification via netsh",
        en_hint="May break on old routers · revert if network issues",
    ),
    SupplementalTweak(
        id="tcp_congestion_ctcp",
        name="TCP CTCP (latency)",
        description="Congestion provider CTCP вместо CUBIC — SpeedGuide",
        category="network",
        risk="medium",
        source="SpeedGuide",
        en_name="TCP CTCP (latency)",
        en_description="Set congestion provider to CTCP for lower latency",
    ),
    SupplementalTweak(
        id="disable_tcp_rsc",
        name="Отключить TCP RSC",
        description="Receive Segment Coalescing OFF — меньше задержка, SpeedGuide",
        category="network",
        risk="safe",
        source="SpeedGuide",
        en_name="Disable TCP RSC",
        en_description="Disable Receive Segment Coalescing for lower latency",
    ),
    SupplementalTweak(
        id="disable_memory_compression",
        name="Отключить сжатие памяти",
        description="Disable-MMACompression — BoosterX / dimaoudegrim",
        category="performance",
        risk="medium",
        requires_reboot=True,
        source="BoosterX · dimaoudegrim",
        hint="Больше RAM занято процессами · полезно при 8 GB",
        en_name="Disable Memory Compression",
        en_description="Disable Windows memory compression (Disable-MMACompression)",
        en_hint="Uses more RAM · useful on 8 GB systems",
    ),
    SupplementalTweak(
        id="wait_to_kill_service_timeout",
        name="WaitToKillServiceTimeout 2s",
        description="Быстрее завершение служб при выключении — BoosterX / FPS packs",
        category="performance",
        risk="medium",
        source="BoosterX · RPG Russia",
        entries=[
            (r"HKLM\SYSTEM\CurrentControlSet\Control", "WaitToKillServiceTimeout", "2000", "5000", winreg.REG_SZ),
        ],
        en_name="WaitToKillServiceTimeout 2s",
        en_description="Faster service shutdown on power-off",
    ),
    SupplementalTweak(
        id="explorer_separate_process",
        name="Explorer: отдельный процесс",
        description="SeparateProcess для папок — dimaoudegrim / WinUtil",
        category="performance",
        risk="safe",
        requires_reboot=False,
        source="dimaoudegrim",
        ...
        en_name="Explorer: Separate Process",
        en_description="Launch folder windows in a separate Explorer process",
    ),
    SupplementalTweak(
        id="increase_icon_cache",
        name="Увеличить кэш иконок",
        description="Max Cached Icons 4096 — dimaoudegrim",
        category="visual",
        risk="safe",
        source="dimaoudegrim",
        entries=[
            (r"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer", "Max Cached Icons", 4096, 500),
        ],
        en_name="Increase Icon Cache",
        en_description="Max Cached Icons 4096 for faster Explorer",
    ),
    SupplementalTweak(
        id="ntfs_disable_compression",
        name="NTFS: без сжатия",
        description="NtfsDisableCompression — меньше CPU на диске",
        category="performance",
        risk="medium",
        requires_reboot=True,
        source="dimaoudegrim",
        entries=[
            (r"HKLM\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsDisableCompression", 1, 0),
        ],
        en_name="NTFS: Disable Compression",
        en_description="Disable NTFS transparent compression",
    ),
    SupplementalTweak(
        id="disable_nearby_sharing",
        name="Отключить Nearby Sharing",
        description="Блокировка «Поделиться с устройствами поблизости»",
        category="privacy",
        risk="safe",
        source="dimaoudegrim",
        entries=[
        ...
        en_name="Disable Nearby Sharing",
        en_description="Block Nearby Sharing feature",
    ),
    SupplementalTweak(
        id="disable_autorun_all",
        name="AutoRun OFF (все диски)",
        description="NoDriveTypeAutoRun 0xFF — dimaoudegrim",
        category="system",
        risk="safe",
        source="dimaoudegrim",
        entries=[
            (r"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer", "NoDriveTypeAutoRun", 255, 145),
        ],
        en_name="Disable AutoRun (all drives)",
        en_description="NoDriveTypeAutoRun 0xFF for all drive types",
    ),
    SupplementalTweak(
        id="disable_edge_startup_boost",
        name="Edge: Startup Boost OFF",
        description="Отключает предзапуск Microsoft Edge",
        category="privacy",
        risk="safe",
        source="dimaoudegrim",
        entries=[
            (r"HKLM\SOFTWARE\Policies\Microsoft\Edge", "StartupBoostEnabled", 0, 1),
            (r"HKLM\SOFTWARE\Policies\Microsoft\Edge", "BackgroundModeEnabled", 0, 1),
        ],
        en_name="Edge: Startup Boost OFF",
        en_description="Disable Microsoft Edge pre-launch and background mode",
    ),
    SupplementalTweak(
        id="verbose_status_messages",
        name="Verbose статус загрузки",
        description="Подробные сообщения при старте/выключении Windows",
        category="system",
        risk="safe",
        source="dimaoudegrim",
        entries=[
            (r"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "VerboseStatus", 1, 0),
        ],
        en_name="Verbose Boot/Shutdown Status",
        en_description="Show detailed status messages during boot and shutdown",
    ),
    SupplementalTweak(
        id="disable_phone_link_policy",
        name="Отключить Phone Link",
        description="Your Phone / Phone Link через политики",
        category="privacy",
        risk="safe",
        source="dimaoudegrim · BoosterX",
        entries=[
            (r"HKLM\SOFTWARE\Policies\Microsoft\Windows\System", "EnableMmx", 0, 1),
            (r"HKLM\SOFTWARE\Policies\Microsoft\Windows\System", "AllowCrossDeviceClipboard", 0, 1),
        ],
        en_name="Disable Phone Link",
        en_description="Disable Phone Link / cross-device clipboard policies",
    ),
]
